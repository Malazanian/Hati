...
module.exports = {
  ...
  execute(message, args) {
    const checkPlayers = (message, playerName) => {
      return message.channel.guild.members
        .filter((member) => member.user.username.toLowerCase().includes(playerName.replace(/"/g, '')))
        ...
        if (buildNumber > 0 && args.length >= 2) {
          ...
          if (getBuild !== null) {
            ...
            if (selectedBuild !== null) {
              let newPlayer = await Build.findOneAndUpdate(
                { _id: buildNumber },
                { $set: { 'group.$[elem].class': group.class } },
                { new: true, arrayFilters: [{ 'elem.name': group.name }] }
              );
              console.log('UpdatedPlayer: ', newPlayer);
            } else {
              let newPlayer = await Build.findOneAndUpdate(
                { _id: buildNumber },
                { $inc: { size: 1 }, $push: { group: group } },
